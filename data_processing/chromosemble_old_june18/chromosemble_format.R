...
df.chain.raw %>% filter(grepl("chromosome_10", refName)) %>% arrange(desc(MaxMatch))

# Remove scaffolds that had a cumultive matching sum < 1kb
df.chain.filtered <- 
  arrange(df.chain.raw, refName, refStart, MaxMatch) %>% 
  mutate(refName = ifelse(MaxMatch < 1000, NA, refName))

df.chain.filtered %>% filter(grepl("chromosome_10", refName)) %>% arrange(desc(MaxMatch))
...
df.good.matches %>% filter(grepl("chromosome_10", refName)) %>% arrange(desc(MaxMatch))
...
chromosomes %>% filter(refName == "chr10") %>% arrange(desc(MaxMatch))
chromosomes %>% filter(refName == "chr7") %>% arrange(desc(MaxMatch))
...
final_chromosomes %>% arrange(desc(MaxMatch))
