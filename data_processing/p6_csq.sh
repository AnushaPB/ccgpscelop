...
ANNOTATION=../../data/genome/annotation/converted_annotation.gff

# Use csq to classify mutations
# Since this is unphased data, we use --local-csq to analyze each variant independently
bcftools csq -f "$GENOME" -g "$ANNOTATION" --local-csq "$VCF" -Ob -o ../../data/genome/annotation/csq.bcf
wc -l outputs/csq.bcf

# Extract all fields (no filtering) to a file
bcftools query -f '%CHROM\t%POS\t%INFO/BCSQ\n' ../../data/genome/annotation/csq.bcf > ../../data/genome/annotation/all_variant_csq.txt
wc -l ../../data/genome/annotation/all_variant_csq.txt