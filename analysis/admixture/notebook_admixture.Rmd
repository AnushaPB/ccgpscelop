...
Q %>% 
  ...
  write_csv(here(outpath, "Q9.csv"))

# write out Q values for the assigned population for Q = 9
Q %>%
  filter(K == 9) %>%
  # only keep the pop group/Q values corresponding to the cluster
  mutate(PopGroup = as.numeric(gsub("V", "", PopGroup))) %>%
...
# Write out K=3 for models
Q %>% 
  # only keep the pop group/Q values corresponding to the cluster
  mutate(PopGroup = as.numeric(gsub("V", "", PopGroup))) %>%
  filter(cluster == PopGroup, K == 3) %>%
  write_csv(here(outpath, "Q3.csv"))
Q %>% 
  # only keep the pop group/Q values corresponding to the cluster
  mutate(PopGroup = as.numeric(gsub("V", "", PopGroup))) %>%
  filter(cluster == PopGroup, K == 5) %>%
  write_csv(here(outpath, "Q5.csv"))
```
...
get_Q(K = 9) %>% 
  ...
  group_by(cluster) %>% 
  group_split() %>%
  iwalk(\(x, idx) write(x$SampleID, here(outpath, paste0("k9_pop", idx, ".txt"))))

# Check population locations
# Read in files and bind with coords 
pop_files <- list.files(here(outpath), full.names = TRUE, pattern = "k9")
coords <- get_coords()
pop_df <- 
  map(pop_files, ~{
    pop <- read_lines(.x)
