...
tess_krig <- function(qmat, coords, grid = NULL, correct_kriged_Q = TRUE) {
  ...
  # Krige each K value
  krig_admix <-
    purrr::map(1:K, ~krig_K(K = .x, qmat, grid, krig_grid, krig_df)) %>%
    purrr::map(~terra::rasterize(.x, grid, field = "var1.pred")) %>%
    terra::rast()
...
#' @param qmat Q matrix
#' @param grid SpatRaster for kriging
#' @param krig_grid sf for kriging
#' @param krig_df sf dataframe with coordinates and Q values for kriging
#'
#' @export
#' @noRd
#' @family TESS functions
krig_K <- function(K, qmat, grid, krig_grid, krig_df) {
