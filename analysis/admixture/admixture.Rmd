...
pdf(here("analysis", "admixture", "admixture.pdf"), width =20, height = 10)
ggplot(df) +
  geom_sf(data = ca) +
...
pdf(here("analysis", "admixture", "admixture_scores.pdf"), width = 10, height = 10)
ggplot(qdf) +
  geom_sf(data = ca) +
...
pop_files <- list.files(here("analysis", "admixture", "outputs"), full.names = TRUE, pattern = "k6")
coords <- get_coords()
pop_df <- 
  map(pop_files, ~{
    pop <- read_lines(.x)
