...
plot(check_model(best_model, check = c("linearity", "vif", "normality"))) # Residuals are very abnormal
dev.off()
```
...
mod_df <- left_join(dframe, mod_df) %>% drop_na(all_of(mod_vars))
# Drop the island coordinate:
coords_drop <- coords %>% filter(SampleID %in% mod_df$SampleID)
...
null.model <- lme(formula(f), data = mod_df, random = ~ 1 | dummy, method = "ML")
...
```{r}
mod_vars <- c("NDVI", "gHM", "paleo_change", "CHELSA_bio1_1981.2010_V.2.1", "csi_past", "glacier", "tmean_dif", "paleo_change_cur_lgm", "Q")
f <- paste("resids ~", paste(paste0(mod_vars), collapse = " + "))
mod <- lm(f, data = mod_df, na.action = "na.fail")
dredge <- MuMIn::dredge(mod)
best_model_f <- MuMIn::get.models(dredge, subset = 1)[[1]]$call
mod <- lm(best_model_f, data = mod_df)
summary(mod)
AIC(mod) # 247.1698
car::vif(mod) # csi_past & paleo_change_cur_lgm VIF > 5, paleo_change > 4

best_mod_vars <- setdiff(all.vars(best_model_f), c("resids", "mod_df"))

pdf(here(plotpath, "sp_resids_all.pdf"), width = 24, height = 6)
...
```{r}
mod_vars <- c("tmean_dif", "csi_past", "Q")
f <- paste("resids ~", paste(paste0(mod_vars), collapse = " + "))
mod <- lm(f, data = mod_df, na.action = "na.fail")
dredge <- MuMIn::dredge(mod)
best_model_f <- MuMIn::get.models(dredge, subset = 1)[[1]]$call
...
```{r}
mod_vars <- c("paleo_change", "csi_past", "tmean_dif", "paleo_change_cur_lgm", "Q")
f <- paste("resids ~", paste(paste0(mod_vars), collapse = " + "))
mod <- lm(f, data = mod_df, na.action = "na.fail")
dredge <- MuMIn::dredge(mod)
best_model_f <- MuMIn::get.models(dredge, subset = 1)[[1]]$call
...
summary(mod)
AIC(mod) # 267.1322
car::vif(mod) # csi_past & paleo_change_cur_lgm VIF > 4

best_mod_vars <- all.vars(best_model_f)[-1]
best_mod_vars <- best_mod_vars[-length(best_mod_vars)]
pdf(here(plotpath, "sp_resids_select.pdf"), width = 20, height = 6)
gglm_ls <- map(best_mod_vars, ~gglm(x = .x, y = "resids", mod_df))
ggpartial_ls <- map(best_mod_vars, ~ggpartial(x = .x, y = "resids", f = best_mod_vars, df = mod_df))
do.call(
  cowplot::plot_grid,
...
### Select variables (contemporary + LGM or LIG)
...
best_mod_vars <- all.vars(best_model_f)[-1]
best_mod_vars <- best_mod_vars[-length(best_mod_vars)]
pdf(here(plotpath, "sp_resids_contemporaryLGM.pdf"), width = 11, height = 6)
gglm_ls <- map(best_mod_vars, ~gglm(x = .x, y = "resids", mod_df))
ggpartial_ls <- map(best_mod_vars, ~ggpartial(x = .x, y = "resids", f = best_mod_vars, df = mod_df))
do.call(
  cowplot::plot_grid,
...
```{r}
mod_vars <- c("gHM", "tmean_dif", "paleo_change", "Q")
f <- paste("resids ~", paste(paste0(mod_vars), collapse = " + "))
mod <- lm(f, data = mod_df, na.action = "na.fail")
dredge <- MuMIn::dredge(mod)
best_model_f <- MuMIn::get.models(dredge, subset = 1)[[1]]$call
...
best_mod_vars <- all.vars(best_model_f)[-1]
best_mod_vars <- best_mod_vars[-length(best_mod_vars)]
pdf(here(plotpath, "sp_resids_contemporaryLIG.pdf"), width = 11, height = 6)
gglm_ls <- map(best_mod_vars, ~gglm(x = .x, y = "resids", mod_df))
ggpartial_ls <- map(best_mod_vars, ~ggpartial(x = .x, y = "resids", f = best_mod_vars, df = mod_df))
do.call(
  cowplot::plot_grid,
...
```{r}
mod_vars <- c("tmean_dif",  "paleo_change_cur_lgm", "paleo_change",  "Q")
f <- paste("resids ~", paste(paste0(mod_vars), collapse = " + "))
mod <- lm(f, data = mod_df, na.action = "na.fail")
dredge <- MuMIn::dredge(mod)
best_model_f <- MuMIn::get.models(dredge, subset = 1)[[1]]$call
...
summary(mod)
AIC(mod) # 323.0065
car::vif(mod) # All VIF < 3

best_mod_vars <- all.vars(best_model_f)[-1]
best_mod_vars <- best_mod_vars[-length(best_mod_vars)]
pdf(here(plotpath, "sp_resids_climatechangeonly.pdf"), width = 12, height = 7)
gglm_ls <- map(best_mod_vars, ~gglm(x = .x, y = "resids", mod_df))
ggpartial_ls <- map(best_mod_vars, ~ggpartial(x = .x, y = "resids", f = best_mod_vars, df = mod_df))
do.call(
  cowplot::plot_grid,
...
```{r}
# Test of residuals
response_resid <- residuals(lm(scale(resids) ~ scale(paleo_change), data = mod_df))
predictor_resid <- residuals(lm(scale(paleo_change) ~ scale(paleo_change_cur_lgm) + scale(tmean_dif), data = mod_df))
cor.test(response_resid, predictor_resid)

mod_vars <- c("NDVI", "gHM", "paleo_change", "CHELSA_bio1_1981.2010_V.2.1", "csi_past", "glacier", "tmean_dif", "paleo_change_cur_lgm", "Q")
cor(predictor_resid, st_drop_geometry(mod_df[, mod_vars]))

paleovars <- get_paleovars()
lgm <- extract(paleovars$lgm, mod_df)[["bio_1"]]
lig <- extract(paleovars$lig, mod_df)[["bio_1"]]

cor.test(mod_df$resid, lgm)
cor.test(mod_df$resid, lig)
cor.test(predictor_resid, lgm)
cor.test(predictor_resid, lig)
```
...
spmod <- build_spatial_models(1000)
...
mod_df2 <- st_join(resids, mod_df)

summary(lm(OLS ~ scale(tmean_dif), data = mod_df2))
summary(lm(SLM ~ scale(tmean_dif), data = mod_df2))
summary(lm(SEM ~ scale(tmean_dif), data = mod_df2))
AIC(lm(OLS ~ scale(tmean_dif), data = mod_df2))
AIC(lm(SEM ~ scale(tmean_dif), data = mod_df2))
AIC(lm(SLM ~ scale(tmean_dif), data = mod_df2))
summary(lm(OLS ~ scale(tmean_dif) + scale(paleo_change) + scale(paleo_change_cur_lgm), data = mod_df2))
summary(lm(SEM ~ scale(tmean_dif) + scale(paleo_change) + scale(paleo_change_cur_lgm), data = mod_df2))
summary(lm(SLM ~ scale(tmean_dif) + scale(paleo_change) + scale(paleo_change_cur_lgm), data = mod_df2))
AIC(lm(OLS ~ scale(tmean_dif) + scale(paleo_change) + scale(paleo_change_cur_lgm), data = mod_df2))
AIC(lm(SEM ~ scale(tmean_dif) + scale(paleo_change) + scale(paleo_change_cur_lgm), data = mod_df2))
AIC(lm(SLM ~ scale(tmean_dif) + scale(paleo_change) + scale(paleo_change_cur_lgm), data = mod_df2))
```

### Test of PC LIG + LGM

```{r}
pc <- prcomp(st_drop_geometry(mod_df[,c("paleo_change_cur_lgm", "paleo_change")]), center = TRUE, scale = TRUE)
mod_df$PC <- pc$x[,1]

mod <- lm(resids ~ PC + tmean_dif, data = mod_df)
summary(mod)

vars <- c("PC", "tmean_dif")
pdf(here(plotpath, "sp_resids_PC.pdf"), width = 8, height = 7)
gglm_ls <- map(vars, ~gglm(x = .x, y = "resids", mod_df))
ggpartial_ls <- map(vars, ~ggpartial(x = .x, y = "resids", f = vars, df = mod_df))
do.call(
  cowplot::plot_grid,
  c(c(gglm_ls, ggpartial_ls), nrow = 2)
)
# Note: don't scale variables for model check
check_model(mod)
dev.off()

mod_df$Sum <- abs(mod_df$paleo_change_cur_lgm) + abs(mod_df$paleo_change)
pdf(here(plotpath, "PC_gendiv_plots.pdf"), width = 2 * 8, height = 8)
cowplot::plot_grid(
  gglm("paleo_change", "Ho", mod_df),
  gglm("paleo_change_cur_lgm", "Ho", mod_df),
  gglm("csi_past", "Ho", mod_df),
  gglm("PC", "Ho", mod_df),
  gglm("paleo_change", "genes_ho", mod_df),
  gglm("paleo_change_cur_lgm", "genes_ho", mod_df),
  gglm("csi_past", "genes_ho", mod_df),
  gglm("PC", "genes_ho", mod_df),
  nrow = 2
)
dev.off()

summary(lm(scale(genes_ho) ~ scale(PC) + scale(tmean_dif), data = mod_df))

summary(lm(scale(genes_ho) ~ poly(scale(PC), 2) + scale(tmean_dif), data = mod_df))
```

### Exploring LGM + LIG weirdness
```{r}

summary(lm(scale(resids) ~ scale(paleo_change_cur_lgm) , data = mod_df))

summary(lm(scale(resids) ~ scale(paleo_change) , data = mod_df))
summary(lm(scale(resids) ~ scale(paleo_change) + scale(paleo_change_cur_lgm), data = mod_df))

car::vif(lm(scale(resids) ~ scale(paleo_change) + scale(paleo_change_cur_lgm), data = mod_df))
