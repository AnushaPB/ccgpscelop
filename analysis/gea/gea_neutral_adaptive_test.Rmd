...
# MI is smaller for null model, so sticking to a linear model
null_M$observed
Msp$observed
...
# Model diagnostics
pdf(here(plotpath, "model_check_resids.pdf"), width = 8, height = 6)
plot(check_model(best_model, check = c("linearity", "vif", "normality")))
dev.off()
```

```{r}
dframe_sf <- dframe %>% st_as_sf() %>% st_transform(3310)
listw <- make_listw(dframe_sf)

sem_result <- spatialreg::errorsarlm(genes_ho ~ neutral_ho, data = dframe, listw = listw, na.action = "na.omit", zero.policy = TRUE)
summary(sem_result)

resids <- residuals(sem_result)
Msp <- ape::Moran.I(resids, spdata.IDW)
Msp$observed
null_M$observed      
```
...
```{r}
mod <- lm(genes_ho ~ neutral_ho, data = dframe)
dframe$resids <- residuals(mod)
```
...
ape::Moran.I(residuals(null.model), spdata.IDW) # significant, but very weak spatial autocorrelation (M = 0.09), so I think it is safe to use a non-spatial model
```
...
summary(mod)
AIC(mod) # 247.1698
car::vif(mod) # cur_lgm = 7

# Drop cur_lgm because of high VIF
mod_vars <- c("NDVI", "gHM", "bio1", "lgm_lig", "csi_past", "glacier", "tmean_dif", "Q")
f <- paste("resids ~", paste(paste0(mod_vars), collapse = " + "))
mod <- lm(f, data = mod_df_scaled, na.action = "na.fail")
dredge <- MuMIn::dredge(mod)
best_model_f <- MuMIn::get.models(dredge, subset = 1)[[1]]$call
...
best_mod_vars <- setdiff(all.vars(best_model_f), c("resids", "mod_df_scaled"))

pdf(here(plotpath, "sp_resids_all.pdf"), width = 19, height = 6)
...
```{r}
mod_vars <- c("csi_past", "cur_lgm", "lgm_lig", "tmean_dif", "Q")
f <- paste("resids ~", paste(paste0(mod_vars), collapse = " + "))
mod <- lm(f, data = mod_df_scaled, na.action = "na.fail")
dredge <- MuMIn::dredge(mod)
best_model_f <- MuMIn::get.models(dredge, subset = 1)[[1]]$call
mod <- lm(best_model_f, data = mod_df_scaled)
summary(mod)
AIC(mod) # 247.1698
car::vif(mod) # csi_past = 5.18, cur_lgm = 7.57

# Drop cur_lgm because of high VIF
mod_vars <- c("csi_past", "lgm_lig", "tmean_dif", "Q")
f <- paste("resids ~", paste(paste0(mod_vars), collapse = " + "))
mod <- lm(f, data = mod_df_scaled, na.action = "na.fail")
dredge <- MuMIn::dredge(mod)
best_model_f <- MuMIn::get.models(dredge, subset = 1)[[1]]$call
...
summary(mod)
AIC(mod) # 247.1698
car::vif(mod) # All VIF < 5

best_mod_vars <- setdiff(all.vars(best_model_f), c("resids", "mod_df_scaled"))
pdf(here(plotpath, "sp_resids_temp.pdf"), width = 15, height = 6)
gglm_ls <- map(best_mod_vars, ~gglm(x = .x, y = "resids", mod_df))
ggpartial_ls <- map(best_mod_vars, ~ggpartial(x = .x, y = "resids", f = best_mod_vars, df = mod_df))
do.call(
  cowplot::plot_grid,
...
dev.off()
```
### Temperature variables + Q only

```{r}
mod_vars <- c("bio1", "csi_past", "cur_lgm", "lgm_lig", "tmean_dif", "Q")
f <- paste("resids ~", paste(paste0(mod_vars), collapse = " + "))
mod <- lm(f, data = mod_df_scaled, na.action = "na.fail")
dredge <- MuMIn::dredge(mod)
best_model_f <- MuMIn::get.models(dredge, subset = 1)[[1]]$call
...
# Drop cur_lgm because of high VIF
mod_vars <- c("bio1", "csi_past", "lgm_lig", "tmean_dif", "Q")
f <- paste("resids ~", paste(paste0(mod_vars), collapse = " + "))
mod <- lm(f, data = mod_df_scaled, na.action = "na.fail")
dredge <- MuMIn::dredge(mod)
best_model_f <- MuMIn::get.models(dredge, subset = 1)[[1]]$call
...
```{r}
pc <- prcomp(st_drop_geometry(mod_df[,c("cur_lgm", "lgm_lig")]), center = TRUE, scale = TRUE)
mod_df$PC <- pc$x[,1]

mod_vars <- c("NDVI", "gHM", "bio1", "PC", "csi_past", "glacier", "tmean_dif", "Q")
f <- paste("resids ~", paste(paste0(mod_vars), collapse = " + "))
...
pdf(here(plotpath, "sp_resids_pc.pdf"), width = 19, height = 6)
gglm_ls <- map(best_mod_vars, ~gglm(x = .x, y = "resids", mod_df))
ggpartial_ls <- map(best_mod_vars, ~ggpartial(x = .x, y = "resids", f = best_mod_vars, df = mod_df))
do.call(
  cowplot::plot_grid,
