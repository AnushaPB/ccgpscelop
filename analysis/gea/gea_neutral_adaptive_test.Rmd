...
convert_lab <- function(x){
  ...
  if (x == "resid") return("Residuals\n(Genome-wide heterozygosity ~ adaptive heterozygosity)")
...
mod_df$neutral_resid <- residuals(lm(neutral_ho ~ paleo_change, data = mod_df))
mod_df$genes_resid <- residuals(lm(genes_ho ~ CHELSA_bio1_1981.2010_V.2.1, data = mod_df))
...
pdf(here("analysis", "gea", "plots", "paleotest.pdf"), width = 22, height = 4.5)
plt1 <- lm_plot(mod_df, "paleo_change", "genes_ho")
plt2 <- lm_plot(mod_df, "CHELSA_bio1_1981.2010_V.2.1", "genes_ho")
plt5 <- lm_plot(mod_df, "paleo_change", "neutral_ho")
plt3 <- lm_plot(mod_df, "neutral_ho", "genes_ho")
plt4 <- lm_plot(mod_df, "neutral_resid", "genes_resid")
cowplot::plot_grid(plt5, plt1, plt2, plt3, plt4, nrow = 1)

plt1 <- sf_plot(sf_df, "paleo_change")
plt2 <- sf_plot(sf_df, "neutral_ho")
plt3 <- sf_plot(sf_df, "genes_ho")
plt4 <- sf_plot(sf_df, "resid_both")
plt5 <- sf_plot(sf_df, "CHELSA_bio1_1981.2010_V.2.1")

cowplot::plot_grid(plt1, plt5, plt2, plt3, plt4, nrow = 1)
dev.off()
...
lm(genes_ho ~ Q + paleo_change + CHELSA_bio1_1981.2010_V.2.1, data = mod_df) %>% broom::tidy()

pdf(here("analysis", "gea", "plots", "univariate_models.pdf"), width = 15, height = 5)
gg_df <- mod_df %>% pivot_longer(c(Q, paleo_change, CHELSA_bio1_1981.2010_V.2.1))
ggplot(gg_df, aes(x = value, y = genes_ho)) + 
  geom_point() + 
  geom_smooth(method = "lm") +
