...
genes_hsapiens %>% count(uniprot_id) %>% filter(n > 1) %>% nrow() #84
...
```{r}
go_hsapien <- run_go(ortho_hsapiens$query, "hsapiens")
...
go_ggallus <- run_go(ortho_ggallus$query, "ggallus")
...
comparison %>%
 ...
 dplyr::select(query_size, intersection_size, query_size_all, intersection_size_all, term_size)
...
comparison %>% filter(grepl("biological regulation", term_name)) %>% dplyr::select(query_size, intersection_size, query_size_all, intersection_size_all)
sig %>% filter(source == "GO:BP") %>% arrange(desc(intersection_size)) %>% head(10)
sig %>% filter(source == "GO:BP") %>% arrange(fish_p) %>% head(10)
```
