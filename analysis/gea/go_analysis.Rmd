...
convert_ortho <- function(query, organism, target_organism){
  ...
  if (source_organism == target_organism) return(query)
  safe_gorth <- possibly(gorth, NA)
...
genes_ortho_human <- 
  genes_org %>%
  drop_na(uniprot_id) %>%
  select(uniprot_id, organism) %>%
  future_pmap(\(uniprot_id, organism) convert_ortho(uniprot_id, organism, target_organism = "hsapiens"), .progress = TRUE) 
plan(sequential)
...
pdf(here(plotpath, "hsapiens.pdf"))
print(p)
dev.off()
...
comparison %>% filter(grepl("biological regulation", term_name)) %>% select(query_size, intersection_size, query_size_all, intersection_size_all)
