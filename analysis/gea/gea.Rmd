...
envdata <- read_csv(here("data", "env", "envdata.csv"))
env <- 
  left_join(coords, envdata) %>% 
  select(CA_rPCA1, CA_rPCA2, CA_rPCA3, elevation) %>% 
  st_drop_geometry() %>%
  data.frame()
...
run_and_export <- imap(f_ls, \(f, idx){
  mod <- vegan::rda(f, data = moddf, scale = TRUE)
  ...
  save(mod, file = here("analysis", "gea", "outputs", paste0(idx, "_rda_mod.rda")))
...
# make x y envstacks
env_agg <- aggregate(envstack, 100)
coords_rast <- as.data.frame(env_agg[[1]], xy = TRUE, ID = FALSE)
x <- env_agg[[1]]
y <- env_agg[[2]]
x[] <- coords_rast$x
y[] <- coords_rast$y
names(x) <- "x"
names(y) <- "y"
env_agg_x_y <- c(env_agg, x, y)

coords_proj <- coords %>% st_transform(crs = st_crs(envstack))
env <- extract(env_agg_x_y, coords_proj, df = TRUE, ID = FALSE)
#env <- extract(env_agg, coords_proj, df = TRUE, ID = FALSE)
env_cci <- which(complete.cases(env))
env_scale <- scale(env, center = TRUE, scale = TRUE)
scale_env <- attr(env_scale, 'scaled:scale')
center_env <- attr(env_scale, 'scaled:center')

# coords
coords_cci <- which(cci)
combo_cci <- intersect(coords_cci, env_cci)
coords_mat <- st_coordinates(coords)
colnames(coords_mat) <- c("x", "y")
gen <- doscc[combo_cci,]
moddf <- data.frame(env[combo_cci,], coords_mat[combo_cci,])

# model
f <- as.formula(paste0("gen ~ ", paste(colnames(env), collapse = "+")))
mod <- vegan::rda(f, data = moddf)

env_pres <- stack(env_agg_x_y)
...
rda_map <- rast(map(res_RDA_proj_current, rast))
plot(rda_map, col = magma(100))
...
rgb <- stack_to_rgb(rda_map)
plotRGB(rgb)
points(coords_proj, col = "black", pch = 1, cex = 1)

ggmaps <- as.data.frame(rgb, xy = TRUE) %>% pivot_longer(cols = -c(x, y), names_to = "variable", values_to = "value")
ggplot(ggmaps) +
  geom_sf(data = ca) +
