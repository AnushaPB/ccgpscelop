...
gdm_plot_diss <- function(gdm_model) {
  # Extract and rename the observed, predicted, and ecological values
  ...
  # Create theoretical GDM overlay curves
  overlayX_ecol <- seq(from = min(dat$ecological), to = max(dat$ecological), length.out = n)
  overlayY_ecol <- 1 - exp(-overlayX_ecol)
  overlayX_pred <- overlayY_pred <- seq(from=min(dat$predicted), to=max(dat$predicted), length.out = n)

  # Plot 1: Observed vs predicted ecological distance
  plot_ecol <-
    ggplot2::ggplot(dat) +
    ggplot2::geom_hex(ggplot2::aes(x = ecological, y = observed)) +
    ggplot2::geom_line(
      data = data.frame(x = overlayX_ecol, y = overlayY_ecol),
  ...
  # Plot 2: Observed vs predicted compositional dissimilarity
  plot_pred <-
    ggplot2::ggplot(dat) +
    ggplot2::geom_hex(ggplot2::aes(x = predicted, y = observed)) +
    ggplot2::geom_line(
      data = data.frame(x = overlayX_pred, y = overlayY_pred),
      mapping = ggplot2::aes(x = x, y = y),
      color = "red", size = 1
    ) +
    ggplot2::theme_classic() +
  ...
  # Combine plots
  cowplot::plot_grid(plot_ecol, plot_pred, nrow = 1)
}
...
gdm_plot_isplines <- function(gdm_model, env, coords, scales = "free", nrow = NULL, ncol = NULL) {
  ...
  # Extract unique x positions for each variable to use as rug lines
  rug_df <- 
    env %>% 
    pivot_longer(everything(), names_to = "name", values_to = "x") %>%
    bind_rows(data.frame(name = "Geographic distance", x = as.vector(st_distance(coords)))) %>%
    mutate(name = gsub("_", " ", name)) 
  yend <- min(gdm_spline_df$y)
  ystart <- yend - (max(gdm_spline_df$y) * 0.05)

  plt <-
    ggplot2::ggplot(gdm_spline_df) +
      # Add small vertical rug lines at bottom
      ggplot2::geom_segment(
        data = rug_df,
        ggplot2::aes(x = x, xend = x, y = ystart, yend = yend),
        alpha = 0.1,
        color = "#7a7a7a"
      ) +
...
gdm_pc_helper <- function(pcaSamp, pcaRast, pcaRastRGB, coords, x = "PC1", y = "PC2", scl = 1, display_axes = FALSE) {
  ...
  varpc <- 
    ...
    mutate(varnames = 
      case_when(
        varnames == "xCoord" ~ "Longitude",
        varnames == "yCoord" ~ "Latitude",
        TRUE ~ varnames)
    )
