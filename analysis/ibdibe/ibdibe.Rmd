...
# MMRR
```{r}
mmrr <- mmrr_do_everything(gendist = gendist, coords = gendist_coords[,c("x", "y")], env = CA_env, quiet = TRUE)

print(mmrr$coeff_df)

# Calculate environmental distances
X <- env_dist(extract(CA_env, gendist_coords[,c("x", "y")]))
# Add geographic distance to X
X[["geodist"]] <- geo_dist(gendist_coords[,c("x", "y")])
Y <- gendist

mmrr_plot(Y, X, mod = mmrr$mod, plot_type = "all", stdz = TRUE)
```
