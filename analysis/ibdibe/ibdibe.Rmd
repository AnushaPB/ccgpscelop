...
coords_proj <- gendist_coords[,c("x", "y")] %>% st_as_sf(coords = c("x", "y"), crs = 4326) %>% st_transform(3310)
...
## Paleoclimate
```{r}
envstack <- rast(here("data", "env", "paleoclim.tif"))
lhstack <- envstack[[grepl("lh", names(envstack))]]
lhpcs <- RStoolbox::rasterPCA(envstack, nComp = 3, scale = TRUE)
lhpcs <- pcs$map
...
curpcs <- RStoolbox::rasterPCA(envstack, nComp = 3, scale = TRUE)
curpcs <- curpcs$map
...
maps <- gdm_map(gdm$model, curpcs, coords_proj, plot_vars = FALSE)
```
...
# MMRR
```{r}
mmrr <- mmrr_do_everything(gendist = gendist, coords = gendist_coords[,c("x", "y")], env = envstack, quiet = FALSE)
