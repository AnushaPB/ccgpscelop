...
# Calculate ROH for all samples at once
./bcftools roh --threads 10 -G30 --AF-dflt 0.4 -O z -o $PREFIX.roh.gz $VCF
zgrep RG $PREFIX.roh.gz > $PREFIX.rg.roh

# Calculate fROH
python -c "from roh import calc_roh; calc_roh('$PREFIX.rg.roh', '$FAI', '$PREFIX.froh')"

# Move data into outputs
mv *$PREFIX* outputs/

# Calculate ROH for each ADMIXTURE cluster (K=5)
