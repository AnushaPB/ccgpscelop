...
df <- extract(env, coords[,c("x", "y")], xy = TRUE)
...
ggplot() +
  ...
  geom_point(data = df, aes(x = x, y = y, fill = envdist), cex = 3, pch = 21, col = "black") +
...
ggplot() +
  geom_raster(data = env_df, aes(x = x, y = y, fill = env1dist)) +
  geom_point(data = df, aes(x = x, y = y, fill = env1dist), cex = 3, pch = 21, col = "black") +
  scale_color_viridis_c(option = "plasma") +
  scale_fill_viridis_c(option = "plasma") +
  theme_void() +
  coord_quickmap()
